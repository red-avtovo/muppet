...
AUTHORIZED_CHAT_ID = int(AUTHORIZED_CHAT_ID_STR) if AUTHORIZED_CHAT_ID_STR.isdigit() else None

# Host and port configuration
HOST = os.environ.get("HOST", "0.0.0.0")  # Default to all interfaces in container
...
async def main():
    ...
    application.add_handler(CommandHandler("getchatid", lambda u, c: u.message.reply_text(f"Your chat ID: {u.effective_chat.id}")))
    ...
    print(f"WebSocket server started on ws://{HOST}:{PORT}")
    print(f"Authorized Chat ID: {AUTHORIZED_CHAT_ID if AUTHORIZED_CHAT_ID else 'No restriction (all chats allowed)'}")
    print(f"Use /getchatid command in Telegram to get your chat ID for configuration")
